...
if(uu.indexOf("baidu.com")!=-1){
    ...
    if(playurl.indexOf("duoduozy.com")!=-1||playurl.indexOf("m3u8.cache.suoyo.cc")!=-1){
      ...
      "web=https://shangjihuoke.com/CL4K/?url="+playurl;
    }else if(playurl.indexOf("tv.nfuxs.club")!=-1){
        var cccc=JZ(JSON.stringify({url:playurl}));
        JSON.stringify({url:JSON.parse(cccc.code).data.url,head:{"User-Agent":"Mozilla/5.0"}});
